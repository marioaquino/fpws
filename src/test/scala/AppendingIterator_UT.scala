...
class AppendingIterator_UT extends FunSpec with ShouldMatchers {
  ...
  describe("appending an iterator") {
     ...
     def secondIterator = Seq(secondItem).iterator
  ...
  describe("The laziness should not evaluate the second iterator before requested") {
